...
import { PlusCircle, Edit, Trash2, Send, RefreshCw, Download } from 'lucide-react';
...
interface Calculation {
  ...
  tbl_resposta_calculo: { resposta_ai: string }[] | null;
  created_at: string;
  [key: string]: any;
}
...
const CalculationListPage = () => {
  ...
  const fetchCalculations = async () => {
    ...
    const { data, error } = await supabase
      .from('tbl_calculos')
      .select('id, nome_funcionario, inicio_contrato, fim_contrato, created_at, tbl_clientes(nome), tbl_resposta_calculo(resposta_ai)')
  ...
  const handleDownloadPdf = (aiResponse: string, fileName: string) => {
    const doc = new jsPDF();
    const margin = 15;
    ...
    doc.setFontSize(12);
    const splitText = doc.splitTextToSize(aiResponse, usableWidth);
  ...
  const handleSendToWebhook = async (calculationId: string, webhookConfigIds: string[]) => {
    // ... (restante da função handleSendToWebhook permanece a mesma)
    if (!user) {
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {calculations.map((calculation) => {
              const aiResponse = calculation.tbl_resposta_calculo?.[0]?.resposta_ai;
              return (
                  ...
                  <CardContent className="flex justify-end space-x-2 w-full">
                    ...
                    {aiResponse ? (
                      <Button
                        ...
                        onClick={() => handleDownloadPdf(aiResponse, `calculo_${calculation.nome_funcionario.replace(/\s+/g, '_')}`)}
                      >
