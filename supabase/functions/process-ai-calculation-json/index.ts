...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!calculationId || !aiResponseJson) {
      return new Response(JSON.stringify({ error: 'Missing calculationId or aiResponseJson' }), {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'Invalid aiResponseJson format', details: (parseError as Error).message }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 400,
      });
    }
    ...
    if (!Verbas_Rescisorias) {
      console.warn('aiResponseJson does not contain Verbas_Rescisorias. Skipping proventos/descontos insertion.');
      return new Response(JSON.stringify({ message: 'No Verbas_Rescisorias found in AI response, skipping detailed insertion.' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200,
      });
    }
    ...
    if (Verbas_Rescisorias.Remuneracao && Array.isArray(Verbas_Rescisorias.Remuneracao)) {
      for (const proventoItem of Verbas_Rescisorias.Remuneracao) {
        ...
        if (valorCalculado > 0 || proventoItem.Natureza_da_Verba === 'Irregularidade_Contratual') {
          ...
          if (insertError) {
            console.error('Error inserting provento:', proventoItem.Provento, insertError);
          }
        }
      }
    }
    ...
    if (Verbas_Rescisorias.Descontos && Array.isArray(Verbas_Rescisorias.Descontos)) {
      for (const descontoItem of Verbas_Rescisorias.Descontos) {
        ...
        if (valorCalculado > 0 || descontoItem.Natureza_da_Verba === 'Irregularidade_Contratual') {
          ...
          if (insertError) {
            console.error('Error inserting desconto:', descontoItem.Desconto, insertError);
          }
        }
      }
    }

    return new Response(JSON.stringify({ message: 'Proventos and descontos processed and stored successfully.' }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
