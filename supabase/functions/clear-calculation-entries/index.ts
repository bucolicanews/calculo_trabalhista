...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (deleteProventosError) {
      ...
      throw new Error(`Failed to delete proventos: ${deleteProventosError.message}`);
    }
    ...
    if (deleteDescontosError) {
      ...
      throw new Error(`Failed to delete descontos: ${deleteDescontosError.message}`);
    }
    ...
    const { error: updateCalculoError } = await supabaseClient
      .from('tbl_calculos')
      .update({ resposta_ai: null })
      .eq('id', calculationId);

    if (updateCalculoError) {
      console.error('Error clearing resposta_ai in tbl_calculos:', updateCalculoError);
      // Não lançar erro fatal aqui, pois a limpeza dos itens já é o principal
    }

    return new Response(JSON.stringify({ message: 'Calculation entries cleared successfully.' }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
